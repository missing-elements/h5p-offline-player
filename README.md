...
handled by downloading the archive into the chunk store first; a host with no CORS headers cannot
be read by any browser, and the element says so with `error: no-cors` so the page can offer a
file picker instead.
...
The real fix belongs to whoever builds the package — storing media instead of deflating it costs
about 1% of file size and makes it seekable immediately.
